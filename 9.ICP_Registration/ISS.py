...
from utils import show_point_color, read_txt
...
def detect_ISS_feature(data:np.ndarray, radius=0.2, radius_NMS=0.5, gamma1=1.5, gamma2=1.5, rate_feature_uper=0.1, rate_feature_lower=0.05):
    '''
    功能：提取ISS特征点
    radius：        对每个点进行半径搜索时的半径，如果等于0则在函数里进行计算，否则直接等于输入的值
    ...
    if radius == 0:
        query = data[0,:].reshape(1,3)
        distance = np.sqrt(np.sum((data-query)**2, axis=1))
        distance.sort()
        radius = distance[30]
    ...
    for index in range(N):
        ...
        if eigenValue[2]==0 or eigenValue[1]==0:
            continue
    ...
    for i in range(10):
        ...
        if num_lambda3 > N*rate_feature_uper*2:
            eigen_riato -= 0.1
        elif num_lambda3 < N*rate_feature_lower*2:
            eigen_riato += 0.02
        else:
            break
    ...
    print("there are {} points satisfied".format(len(output_index)))
